...
# log : 会話の履歴(list[{}])
# child_msg : 親の発言(str)
def parent_obs(log, child_msg, child_action):
    # 初期値設定（会話の例）
    if log == []:
        # 深津式プロンプト
        system_message = """
        ...
        # 制約条件
        ・入力文は全て子供からあなたへの発言です。
        ・子供の行動は[Do(掃除する), Don't(掃除しない)]の2種類に分類されます。
        ...
        log = [
            ...
            {"role": "user", "content": "もうやだ！部屋の掃除なんて面倒くさい！ (Don't)"},
            {"role": "assistant", "content": "-6 7"},
        ]

    # メッセージ設定
    user_message = [{"role": "user", "content": f"{child_msg} ({child_action})"}]
    ...
    for try_time in range(try_count):
        try:
            ...
            if (
                ...
                print(f"Parent observation : {russell}")
                break
