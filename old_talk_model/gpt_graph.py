...
sli_a = wg.Slider(ax_a, "parent action", 1, 3, valinit=v.a, valstep=1)
sli_parent_val = wg.Slider(ax_parent_val, "parent valence", -10, 10, valinit=v.parent_valence, valstep=1)
sli_parent_aro = wg.Slider(ax_parent_aro, "parent arousal", -10, 10, valinit=v.parent_arousal, valstep=1)
sli_child_val = wg.Slider(ax_child_val, "child valence", -10, 10, valinit=v.child_valence, valstep=1)
sli_child_aro = wg.Slider(ax_child_aro, "child arousal", -10, 10, valinit=v.child_arousal, valstep=1)
...
def btn_do_click(event):
    ...
    init_data = np.array({'time':v.t, 'act':v.a, 'parent_val':v.parent_valence, 'parent_aro':v.parent_arousal, 'child_val':v.child_valence, 'child_aro':v.child_arousal})
    np.save("./data/init_data", init_data)

    data_name = ["parent_obs", "parent_state", "child_obs", "child_state"]
    # モデル実行
    all_history = gpt_talk.main(v.t, v.a-1, [v.parent_valence, v.parent_arousal], [v.child_valence, v.child_arousal])
    ...
    for i, ax in enumerate(graphs):
        ...
        np.save(f"./data/{data_name[i]}", history)
        plt.draw()
